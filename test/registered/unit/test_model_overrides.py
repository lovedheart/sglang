...
class TestGoldenModelOverrides(_IsolatedPublish):
    ...
    def test_qwen4_ple_offload_default(self):
        ...
        with override_platform(is_cuda=True):
            for kwargs, expected in (
                ...
                with self.subTest(kwargs=kwargs):
                    self.assertEqual(
                        self._resolved(self._construct(*qwen4, **kwargs),
                                       "ple_offload_embedding"),
                        expected,
                    )
        ...
        with override_platform(is_cuda=False):
            # disable_radix_cache keeps the mamba extra_buffer platform
            # assertion (irrelevant to this check) from gating the CPU case.
            self.assertFalse(
                self._resolved(
                    self._construct(*qwen4, disable_radix_cache=True),
                    "ple_offload_embedding",
                )
            )
